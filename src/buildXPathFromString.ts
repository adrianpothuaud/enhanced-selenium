import { buildXPathFromArrayOfObjects } from './buildXPathFromArrayOfObjects'
import { EAcceptedXPathObjectKeys, EAcceptedXPathStringKeys } from './enums'
import { IXPathBuilderItem } from './types'

function evaluateItemAttributes(notParsedAttr: string, parsedItem: IXPathBuilderItem) {
  const [key, value] = notParsedAttr.split(':'),
    refinedKey: keyof EAcceptedXPathObjectKeys | string = {
      [EAcceptedXPathStringKeys.ID]: 'id',
      ['*.']: 'containsClass',
      ['*txt']: 'textContains',
      ['*val']: 'valueContains',
      ['.']: 'class',
 ...
 */
export function buildXPathFromString(item: string) {
  ...
  for (let notParsedItemsIndex = 0; notParsedItemsIndex < notParsedItems.length; notParsedItemsIndex++) {
    ...
    const parsedItem: IXPathBuilderItem = {}
    let hasTag = false
    ...
    if (parsedItem.tag) parsedItem.tag = {
      ['btn']: 'button',
      ['d']: 'div',
    }[parsedItem.tag] || parsedItem.tag
    parsedItems.push(parsedItem)
  }
  const finalXPath = buildXPathFromArrayOfObjects(parsedItems)
  return finalXPath
}
