...
exports.buildXPathFromArrayOfObjects = void 0;
const enums_1 = require("./enums");
const acceptedUnitAttrs = Object.keys(enums_1.EAcceptedXPathStringKeys);
function itemHasAtLeastOneAttr(item) {
    for (let index = 0; index < Object.keys(item).length; index++) {
        const key = Object.keys(item)[index];
        if (acceptedUnitAttrs.includes(key))
            return true;
    }
    return false;
}
function buildXPathFromArrayOfObjects(items) {
    const builtXPathElements = [];
    for (let i = 0; i < items.length; i++) {
        const item = items[i];
        const unitXPath = ['//'];
        if (item.tag)
            unitXPath.push(item.tag);
        else
            unitXPath.push('*');
...
exports.buildXPathFromArrayOfObjects = buildXPathFromArrayOfObjects;
